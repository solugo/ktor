...
/**
 * Plugin that provides retry functionality. Has no retry policy by default.
 * Users can provide custom retry and delay policies or use one of the predefined.
 */
public class HttpRequestRetry internal constructor(configuration: Configuration) {

    /**
     * Data for [HttpRequestRetryEvent] event. May contain [response] or [cause], but not both
     */
    ...
    private var maxRetries: Int = configuration.maxRetries
    ...
    public class Configuration {
         ...
         */
        public fun retryIf(maxRetries: Int = 3, block: (HttpRequest, HttpResponse) -> Boolean) {
         ...
         * and should return `true` if this plugin need to retry this request of `false` otherwise
         */
        public fun retryOnExceptionIf(maxRetries: Int = 3, block: (HttpRequestBuilder, Throwable) -> Boolean) {
            this.maxRetries = maxRetries
         ...
         */
        public fun retryOnException(maxRetries: Int = 3) {
         ...
         */
        public fun retryOnServerErrors(maxRetries: Int = 3) {
         ...
         * or 5XX responses was received
         */
        public fun retryOnExceptionOrServerErrors(maxRetries: Int = 3) {
        ...
        /**
         * Constant delay of `millis + randomizationMs` milliseconds
         */
         ...
         * pattern. Retries will be calculated using `base ^ retryCount + randomizationMs`
         */
        ...
        client[HttpSend].intercept { request ->
            ...
            while (true) {
                ...
                val retryData = try {
                    ...
                    val retry = retryCount < maxRetries && shouldRetry(call.request, call.response)
                    if (!retry) break
                    RetryEventData(subRequest, ++retryCount, call.response, null)
                } catch (cause: Throwable) {
                    val retry = retryCount < maxRetries && shouldRetryOnException(subRequest, cause)
