...
private class RequestHolders(
    ...
    val responseReady: CompletableDeferred<CurlSuccess>
) {
    fun dispose() {
        response.dispose()
    }
...
internal class CurlMultiApiHandler : Closeable {
    private val activeHandles: MutableMap<EasyHandle, RequestHolders> = ConcurrentMap()
    ...
    val easyHandlesToUnpause by shared(ConcurrentList<EasyHandle>())
    ...
    private fun handleCompleted() {
        for (cancellation in cancelledHandles) {
            ...
            handler.responseReady.completeExceptionally(cancelled.cause)
            handler.dispose()
        }
        ...
        memScoped {
            do {
                ...
                try {
                    ...
                    val deferred = activeHandles[easyHandle]!!.responseReady
